...
- [x] A deterministic GitHub write-policy guard now rejects repository, branch, SHA, operation, path, file-count, and byte-limit violations before any future GitHub client call. A private read-only MCP service now exposes `get_pull_request`, `get_pull_request_files`, `get_file`, and `get_checks`; their mocked Streamable HTTP flows and client calls are verified.
