...
describe("GitHub MCP server", () => {
  ...
  it("serves separate stateless HTTP requests", async () => {
    ...
    const getFile = vi.fn(async (path: string, ref: string) => ({ lineNumberedContent: "1 | source", path, ref }));
    ...
    await expect(
      ...
      content: [{ text: JSON.stringify({ lineNumberedContent: "1 | source", path: "apps/forgegate/src/payment-lab.ts", ref: "a".repeat(40) }), type: "text" }],
    });
    await expect(
