...
const continuationPrompts = [
  ...
  "Continue with Phase EXPERIMENT and EVIDENCE. Resolve master to its immutable SHA and generate one temporary scenario runner from each accepted ScenarioPlan. Run the same runner on master before checking out the exact PR SHA, then run it on the PR SHA with the same scenarioId and seed. Return one schema-valid generic ExperimentResult per scenario with baselineSha, scenarioId, repetitions, expected baseline values, observed PR values, verdict, and concrete sandbox artifact links. Return UNCERTAIN when the repository cannot express or execute a scenario.",
      ...
      input: [
        {
          content: [
            ...
            "The invariant-analyst delegated input must explicitly allow only forgegate-github get_file for approved repository paths at the exact PR head SHA; it must use lineNumberedContent from those MCP responses for evidence locations, then stop using tools.",
            ...
            "Scenario actions and assertions must be derived from the repository evidence; do not invent operations the checked-out repository cannot execute.",
            ...
            "Resolve master to its immutable SHA and run each generated scenario runner before PR checkout. Every ExperimentResult must include baselineSha for that master commit, use its measured values as expected, use PR-head values as observed, and mark verdict fail when the observed values violate an accepted invariant.",
