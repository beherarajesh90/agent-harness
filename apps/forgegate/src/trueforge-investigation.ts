      ...
      input: [
        {
          content: [
            ...
            "The invariant-analyst delegated input must explicitly allow only forgegate-github get_file for the two approved payment-lab paths at the exact PR head SHA; after those MCP responses, it must reason from returned content and stop using tools.",
            "The failure-mode-analyst delegated input must explicitly state that it has no tools and must not call list_tools, MCP, exec, shell, Python, Git, or sandbox; it must reason only from the validated invariant JSON.",
