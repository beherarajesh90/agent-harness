...
const continuationPrompts = [
  ...
  "Continue with Phase EXPERIMENT and EVIDENCE. Resolve master to its immutable SHA and generate one temporary scenario runner from each accepted ScenarioPlan. First run a scenario-independent baseline on master without injected faults, then run the same scenario runner on the PR SHA with the same scenarioId and seed. Copy the same baseline measurements into every ExperimentResult as expected values; use PR-head values as observed. Return one schema-valid generic ExperimentResult per scenario with baselineSha, scenarioId, repetitions, verdict, and concrete sandbox artifact links. Return UNCERTAIN when the repository cannot express or execute a scenario.",
