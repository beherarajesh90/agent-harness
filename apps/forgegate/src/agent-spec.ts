...
export function createForgeGateAgentSpec(modelName: string): AgentSpec {
  ...
  return {
    ...
    instructions: [
      ...
      "When creating invariant-analyst, explicitly state that it may call only forgegate-github get_file for the two approved payment-lab paths at the exact PR head SHA; after those MCP responses, it must reason from the returned content and stop using tools.",
